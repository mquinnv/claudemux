...
func TestComputePreviewLayout(t *testing.T) {
	...
	tests := []struct {
		...
		{"a full-screen lobby", 46, false, false, 99, swLayout{Show: true, Content: 13, ListRows: 25}},
		// avail 18 -> 6 content (floor), list 18-8 = 10.
		{"a small lobby hits the floor", 24, false, false, 99, swLayout{Show: true, Content: 6, ListRows: 10}},
		// avail 94 -> 31 clamped to 16, list 94-18 = 76.
		{"a tall lobby hits the ceiling", 100, false, false, 99, swLayout{Show: true, Content: 16, ListRows: 76}},
		// avail 11 -> 6 content, list 3: the last height that fits both
		// (list=3 -> budget 2 -> one 2-row session + the more line).
		{"the smallest lobby with a preview", 17, false, false, 99, swLayout{Show: true, Content: 6, ListRows: 3}},
		// avail 10 -> list would be 2, one short of a full session row
		// surviving the cap: fleet wins, preview is dropped.
		{"one row short: fleet wins", 16, false, false, 99, swLayout{}},
		// avail 9 -> list would be 1: fleet wins, preview is dropped.
		{"too short for both", 15, false, false, 99, swLayout{}},
		{"no size yet", 0, false, false, 99, swLayout{}},
		// An error line costs a row: avail 39 -> 13 content, list 39-15 = 24.
		{"an error line shifts the budget", 46, true, false, 99, swLayout{Show: true, Content: 13, ListRows: 24}},
		// A small fleet returns its unused rows to the preview: avail 40,
		// claim 13, list share 25, fleet only needs 4 -> content 13+21 = 34.
		{"a small fleet grows the preview", 46, false, false, 4, swLayout{Show: true, Content: 34, ListRows: 4}},
		...
		{"growth is not capped at the ceiling", 100, false, false, 4, swLayout{Show: true, Content: 88, ListRows: 4}},
		// A fleet exactly filling its share changes nothing.
		{"an exactly-fitting fleet", 46, false, false, 25, swLayout{Show: true, Content: 13, ListRows: 25}},
		...
		{"a tiny fleet fits where a big one would not", 16, false, false, 2, swLayout{Show: true, Content: 6, ListRows: 2}},
		// The meters line costs a row, same as the error line: avail 39 ->
		// 13 content, list 39-15 = 24.
		{"a meters line shifts the budget", 46, false, true, 99, swLayout{Show: true, Content: 13, ListRows: 24}},
		// Both extra lines together: avail 38 -> 12 content, list 38-14 = 24.
		{"meters and error lines stack", 46, true, true, 99, swLayout{Show: true, Content: 12, ListRows: 24}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
